...
def right_button(app_name,version,path,path2):
    file_name=app_name+" Right.ini"
    image_name=app_name+" Right.png"
    side="Right"
    file=open(file_name,'w')
    file.write('[Rainmeter]\nUpdate=1000\nDynamicWindowSize=1\nAccurateText=1\nSkinWidth=180\nSkinHeight=50\nBackgroundMode=2\nSolidColor=0,0,0,1\n\n')
    file.write('[Metadata]\nName=unFold\nAuthor=feedmes1eep\nVersion= {}\nLicense=GNU General Public License v2.0\nInformation=The right side button for {}\n\n'.format(version,app_name))
...
left_button(app_name,ver,path,path2)
right_button(app_name,ver,path,path2)